...
//				if($_SESSION['loginStatus'] || $_REQUEST['aj_action'] == 'login'){
...
//				}
	...
	/*private function login() {
		...
		$userPassword = isset($_POST['pass_word'])?$_POST['pass_word']:'';
		$response = array();

		if($userName != '' && $userPassword != ''){
						
			$sqlText = "
				SELECT idUser,userName,userPassword,userEmail
				FROM translations.users
				WHERE
					((userName = '$userName' AND userPassword = '$userPassword')
					OR
					(userEmail = '$userName' AND userName = '$userName'))
					AND active = 1";						
			...
			$this->ajaxResponse(0,"","FORM",null,$response);
		}
		else{
			$this->ajaxResponse(3001,"invalid request","FORM", null,$response);
		}
	}	*/
