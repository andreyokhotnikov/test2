//-----------------------------------------grid-filterbar--begin--------------------------------------------------------
//20210901--https://github.com/Druid33/ext6-grid-filterbar
...
Ext.define('GridFilterBar.grid.FilterBar', {
    ...
    showHideButtonTooltipDo     : 'Поквзать&nbsp;панель&nbsp;фильтров',//'Show&nbsp;filter&nbsp;bar',    // button tooltip show
    showHideButtonTooltipUndo   : 'Скрыть&nbsp;панель&nbsp;фильтров',//'Hide&nbsp;filter&nbsp;bar',    // button tooltip hide
    ...
    clearAllButtonTooltip       : 'Очистить&nbsp;все&nbsp;фильтры',//'Clear&nbsp;all&nbsp;filters',  // button tooltip
    ...
    listTpl: {
        ...
        editable: false,
        triggerAction: 'all',
        multiSelect: false, //A: list options
        operator: 'like'	//A: list options
//original:		
//        multiSelect: true,
//        operator: 'in'
    ...
    unsetup: function() {
        ...
        filterData.fields.each(function(item) {
			//Ext.destroy(item);//20210903 Andrey 
        });
        ...
        Ext.each(columns, function(column) {
            if (column.filter) {
                ...
                if (column.filter.type) {
					...
					/*else if (column.filter.type == 'list' && !column.filter.store) {
                ...
                if (column.filter.xtype == 'combo' && !column.filter.store) {
                    ...
                    //filterData.autoStores.add(column.dataIndex, column.filter.store);	//original
					filterData.autoStores.add(column.dataIndex, column.filter);			//A: list options
    ...
    fillAutoStores: function() {
        ...
        //if (!filterData.autoUpdateAutoStores && filterData.autoStoresLoaded) { //A: list options
        //    return;
        //}
            ...
            if (!grid.store.remoteFilter) { // values from local store
                ...
                Ext.each(data, function(id) {				
                    if (Ext.isEmpty(id)) {
						...
						var optionsArr = item.options;						//A: list options
						if(typeof(optionsArr) !== "undefined"){
							var find = optionsArr.find(x => x.id === id);
							if (find) {
								var txt = (find.text)?find.text:id;
								var cls = (find.cls)?find.cls:'';
							} else {
								var txt = id;
								var cls = "";
							}
                ...
                //item.loadData(records);		//original
				item.store.loadData(records);	//A: list options
                ...
                if (grid.store.proxy.reader.rawData[filterData.autoStoresRemoteProperty]) {
                    ...
                    if (data[key]) {
                        records = [];
                        Ext.each(data[key].sort(), function(id) {
                            if (Ext.isEmpty(id)) {
                                Ext.Array.insert(records, 0, [{
                                    text: filterData.autoStoresNullText,
                                    id: filterData.autoStoresNullValue
        ...
        if (filterData.columns.getCount() && !filterData.actionColumn && (filterData.showClearAllButton || filterData.showShowHideButton)) {
            ...
            if (!document.getElementById(extraColumnCssClass)) {
                var style = document.createElement('style'),
                    //css = 'tr.' + Ext.baseCSSPrefix + 'grid-row td.' + extraColumnCssClass + ' { background-color: #ffffff !important; border-color: #ffffff !important; }'; //A
					css = 'tr.' + Ext.baseCSSPrefix + 'grid-row td.' + extraColumnCssClass + ' { border-color: #ffffff !important; }';
            ...
            if (!grid.store.remoteFilter) {
                ...
                switch(operator) {
                    case 'eq':
                        filterFn = function(item) {
                            if (column.filter.type == 'date') {
								if (Ext.isDate(item.get(column.dataIndex))) {//A
                    ...
                    case 'gte':
                        filterFn = function(item) {
                            if (column.filter.type == 'date') {
								if (Ext.isDate(item.get(column.dataIndex))) {//A //20210921
									return Ext.Date.clearTime(item.get(column.dataIndex), true).getTime() >= Ext.Date.clearTime(newVal, true).getTime();
								}
                            } else {
                                return (Ext.isEmpty(item.get(column.dataIndex)) ? filterData.autoStoresNullValue : item.get(column.dataIndex)) >= (Ext.isEmpty(newVal) ? filterData.autoStoresNullValue : newVal);
